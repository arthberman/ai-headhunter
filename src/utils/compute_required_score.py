...
def compute_required_score(
    ...
    """Compute the required score heuristically."""
    # Get scored criteria for Must
    scored_required_criteria = get_extended_scored_criterion(
        scored_criterion,
        scorecard,
    ...
    heuristic_result = Decision(
        type=DecisionType.ALL_REQUIRED_CRITERIA,
        outcome=Outcome.REJECTED,
        explanation="",
    )
    scores = [criterion.score for criterion in scored_required_criteria]
    confidence_scores = [criterion.confidence for criterion in scored_required_criteria]
    ...
    if min_score >= 0.6:
        heuristic_result = Decision(
            ...
            explanation="All criteria meet minimum threshold",
        )
    elif min_score <= 0.3:
        heuristic_result = Decision(
            type=DecisionType.ALL_REQUIRED_CRITERIA,
        ...
        heuristic_result = Decision(
            type=DecisionType.ALL_REQUIRED_CRITERIA,
            outcome=Outcome.REVIEW,
            explanation="Typical performance is borderline",
        )
    else:
        heuristic_result = Decision(
            type=DecisionType.ALL_REQUIRED_CRITERIA,
    ...
    if avg_confidence < 0.6 and heuristic_result.outcome == Outcome.ACCEPTED:
        heuristic_result = Decision(
            type=DecisionType.ALL_REQUIRED_CRITERIA,
            outcome=Outcome.REVIEW,
            explanation="Low confidence in assessment",
        )
