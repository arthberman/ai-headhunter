...
def results_synthesizer(state: OverallState) -> OverallState:
    ...
    try:
        ...
        if state.query_results is None:
            logger.error("No query results found in state")
            raise ValueError("No query results found in state")
        ...
        for query in state.query_results:
            ...
            for iteration in query.iterations:
                if iteration.count is None:
                    logger.error("No count found for iteration")
                    raise ValueError("No count found for iteration")
        ...
        logger.error(f"Error in results_synthesizer: {e}")
        return state
