...
from src.feeder.subgraph.process_not_in_job_titles.models.feedback_judge import (
    FeedbackResponse,
)
...
async def judge_existing_not_in_job_titles(
    state: JobsToExcludeState,
) -> dict[str, FeedbackResponse]:
    """Judge of the quality of each job title inside the existing job titles to exclude.
    ...
    try:
        ...
        logger.info("Successfully judged existing not_in_job_titles")
        return {"feedback_judge": res}
    except Exception as e:
        logger.error(f"Error in judge_existing_not_in_job_titles: {e}")
        raise e
