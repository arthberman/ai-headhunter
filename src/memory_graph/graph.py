...
async def handle_patch_memory(
    ...
    existing_item = await store.aget(state.namespace, state.key)
    ...
    # Save to storage
    await store.aput(state.namespace, state.key, extracted)
...
async def handle_insertion_memory(
    ...
    existing_items = await store.asearch(state.namespace, limit=5)
