...
def get_decision_subgraph():
    ...
    workflow.add_node(
        "assess_open_to_work", node_decision_open_to_work, retry=get_retry_policy()
    )
    workflow.add_node(
        "check_hierarchy", node_decision_hierarchy_move, retry=get_retry_policy()
    )
    workflow.add_node(
    ...
    workflow.add_node(
        "synthetize_intent", node_decision_intent_to_move, retry=get_retry_policy()
    )
    workflow.add_node(
