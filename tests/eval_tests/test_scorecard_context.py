...
def predict_context(example: dict):
    state = ScorecardGraphState(**example)
    res = node_context(state)
...
def judge_evaluator_criteria(root_run: Run, example: Example) -> dict:
    ...
    prompt = get_prompt("eval-judge-scorecard-context-criteria")
...
experiment_results = evaluate(
    predict_context,
    data="ds-scorecard-context",
    evaluators=[judge_evaluator_criteria],
    experiment_prefix="test-scorecard-context",
    metadata={},
)
